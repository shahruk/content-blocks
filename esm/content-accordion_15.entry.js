import { r as registerInstance, h, g as getElement, c as createEvent, a as getCurrentValue, H as Host, d as generateResponsiveClass, e as breakpointValues, f as getValueAtIndex, j as breakpoints } from './utils-bec23d2d.js';
import { W as WindowResizeService, B as BehaviorSubject } from './window-resize-service-02472872.js';
...
const ContentCarousel = class {
  ...
  async initSwiper() {
    let Swiper = await import('./swiper.esm-31bc2e0c.js');
  ...
  render() {
    return (h("content-block-wrapper", { name: "content-carousel", background: this.background }, h("div", { class: "content-carousel__content" }, h("div", { class: "swiper-container", ref: el => this.swiperContainer = el }, h("div", { class: "swiper-wrapper", ref: el => this.swiperWrapper = el }, h("slot", { name: "content" }), h("slot", null)), h("div", { class: "swiper-pagination", ref: el => this.swiperPagination = el }), h("div", { class: "swiper-button-prev", ref: el => this.swiperPrevElement = el }), h("div", { class: "swiper-button-next", ref: el => this.swiperNextElement = el }), h("div", { class: "swiper-scrollbar", ref: el => this.swiperScrollbar = el })))));
  }
  get hostElement() { return getElement(this); }
};
...
const ContentVideo = class {
    ...
    let observer = new IntersectionObserver((entries) => {
      entries.forEach(async (entry) => {
        if (entry.isIntersecting) {
          let videojs = await import('./video.es-e5aa03db.js').then(function (n) { return n.v; });
          await import('./Youtube-7319462c.js').then(function (n) { return n.Y; });
  ...
  render() {
    return (h(Host, { class: this.activeClass }, (this.hasPoster && !this.isLoaded) &&
      h("div", { class: "content-video__poster" }, h("slot", { name: "poster" })), ((this.hasPoster && !this.isPlayed) || !this.isPlayed) &&
      h("button", { class: "content-video__play", "aria-label": this.playButtonAriaLabel, onClick: ev => this.onClickHandler(ev) }, h("slot", { name: "playButton" })), h("div", { class: "content-video__content" }, (!this.hasPoster || this.isLoaded) &&
      h("video", { ref: element => this.videoElement = element, class: "video-js vjs-content-block-theme", controls: this.controls, preload: this.preload, width: this.width, height: this.height, crossorigin: "anonymous" }, h("source", { src: this.src, type: this.type }), h("slot", { name: "no-js-message" }), h("slot", { name: "content" }), h("slot", null)))));
  }
  get hostElement() { return getElement(this); }
};
ContentVideo.style = contentVideoCss;

export { ContentAccordion as content_accordion, ContentBlockFormatter as content_block_formatter, ContentBlockWrapper as content_block_wrapper, ContentButton as content_button, ContentCarousel as content_carousel, ContentFormatterWrapper as content_formatter_wrapper, ContentGridFormatter as content_grid_formatter, ContentHeroBlock as content_hero_block, ContentImage as content_image, ContentBlockLine as content_line, ContentSplitBlock as content_split_block, ContentTabs as content_tabs, ContentTextBlock as content_text_block, ContentToutBlock as content_tout_block, ContentVideo as content_video };
